...
public class GoogleTOTP {
	 ...
	 */
	public GoogleTOTP(Mac mac) {
		this(mac, PASS_CODE_LENGTH);
	}

	public GoogleTOTP(Signer signer) {
	 ...
	 */
	public GoogleTOTP(final Mac mac, int passCodeLength) {
		this(new Signer() {
			@Override
	...
	public GoogleTOTP(Signer signer, int passCodeLength) {
		if ((passCodeLength < 0) || (passCodeLength > MAX_PASSCODE_LENGTH)) {
			throw new IllegalArgumentException("PassCodeLength must be between 1 and " + MAX_PASSCODE_LENGTH + " digits.");
		}
		this.signer = signer;
	...
	public String generateResponseCode(long state, byte[] challenge) throws GeneralSecurityException {
		if (challenge == null) {
			...
			byte value[] = ByteBuffer.allocate(8 + challenge.length).putLong(state) // Write
																					// out
																					// OTP
																					// state
	...
	public boolean verifyTimeoutCode(long currentInterval, String timeoutCode) throws GeneralSecurityException {
		return verifyTimeoutCode(timeoutCode, currentInterval, ADJACENT_INTERVALS, ADJACENT_INTERVALS);
	}
